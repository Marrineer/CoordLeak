        ...
        switch (subCommand) {
            ...
            case "share":
                ...
                private void handleReload(CommandSender sender, String[] args) {
                    ...
                    // Cooldown for reload command
                    if (protectionManager.isOnCooldown(player.getUniqueId(), "reload")) {
                        long remaining = protectionManager.getRemainingCooldown(player.getUniqueId(), "reload");
                        messageUtil.sendToSender(plugin.getMessageManager().getCommandCooldown().replace("%time%", formatTime(remaining)), player);
                        return;
                    }
            
                    // Rate limit for reload command
                    if (protectionManager.isRateLimited(player.getUniqueId(), "reload")) {
                        messageUtil.sendToSender(plugin.getMessageManager().getCommandRateLimited(), player);
                        auditLogger.log("RELOAD_ATTEMPT", player.getName(), "N/A", 0, "RATE_LIMITED", player.getAddress() != null ? player.getAddress().getAddress().getHostAddress() : "N/A", "Player tried to reload too fast.");
                        return;
                    }
                    ...
                    plugin.reloadManagers();
                    protectionManager.applyCooldown(player.getUniqueId(), "reload");
                    protectionManager.recordRateLimitUsage(player.getUniqueId(), "reload");
                ...
                private void handleSetPrice(CommandSender sender, String[] args) {
                    ...
                    if (args.length < 2) { // Expecting /coord setprice <amount>
                        ...
                        messageUtil.sendToSender(plugin.getMessageManager().getCurrentPrice().replace("%price%", String.format("%.2f", currentPrice)), sender);
                        return;
                    }
